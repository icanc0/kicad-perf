...
| 46 | `0046-sch-upgrade-skip-connectivity.patch`                                        | Same for `JobUpgrade` (sch upgrade): parses .kicad_sch and re-serializes to current version; no downstream consumer reads the ConnectionGraph. | same as 0045 | trivial |
| 47 | `0047-netinfo-rebuild-display-names-memo.patch`                                   | `NETINFO_LIST::RebuildDisplayNetnames` was O(N²·d) — the inner loop wxSplit-ed every long-name inside every outer iteration for the same short-name group. Cache the pre-split parts on the group entry; drops to O(N·d). | noticeable on hierarchical schematics with many nets sharing a short name (GND/VCC etc.) | trivial |
| 48 | `0048-sch-sheet-list-instance-lookup-map.patch`                                   | `SCH_SHEET_LIST::UpdateSymbolInstanceData` and `UpdateSheetInstanceData` did a `std::find_if` over the full instance vector for every path — O(N·M). Build a `std::map<KIID_PATH, instance*>` up front, O(log M) lookups thereafter. | **O(N·M) → O((N+M)·log M)** on schematic load; big win on hierarchical designs with 1000+ symbols | trivial |
| 49 | `0049-sch-screen-prune-orphans-precompute-lookup.patch`                            | `SCH_SCREEN::PruneOrphaned{Symbol,Sheet}Instances` walked `GetSheetPathByKIIDPath` (linear scan over full sheet list) once per instance per item — O(Y·I·P). Precompute a `std::map<KIID_PATH, SCH_SHEET_PATH>` once per call, drop per-item lookup to O(log P). | **~100× algorithmic** on the orphan-prune phase of schematic load, on hierarchical designs | trivial |
| 50 | `0050-sch-screens-share-orphan-prune-lookup.patch`                                 | Follow-up to 0049 — hoist the lookup-map construction from per-screen to the SCH_SCREENS driver. Add map-taking overloads on SCH_SCREEN; SCH_SCREENS builds once and passes to every screen. Total build cost drops from O(S·P·log P) to O(P·log P). | small on top of 0049; more meaningful the deeper the sheet hierarchy | trivial |
| 51 | `0051-annotate-power-symbols-fix-and-vs-or.patch`                                  | `SCH_SHEET_LIST::AnnotatePowerSymbols` had `curr_ref.Last() < '0' && curr_ref.Last() > '9'` — impossible for a single char to satisfy, so the "skip unannotated refs" fast-path was dead code. Fix to `\|\|`. Behaviour unchanged (the digit-strip loop below was a no-op on unannotated refs anyway) but intent is now expressed correctly. | correctness / intent | trivial |

